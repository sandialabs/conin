...
class DBNSampler:
    ...
    def extract_parents(self, cpd):
        ...
        return [
            (
                (_parent[0], _parent[1].value() == (self.G.t.value() - 1))
                if isinstance(_parent, tuple)
                else ((_parent, True))  # dynamic  # static
            )
    ...
    def get_children(self, node):
        ...
        return [
            ...
            for _parent in (_cpd.parents or [])
            if (
                (_parent[0] == node and _parent[1].value() == self.G.t.value())
                if isinstance(_parent, tuple)
                else _parent == node
            )
        ]
    ...
    def sample_next_states(self, previous_states={}):
        ...
        degree_map = {
            _node: len(self.representation[_node]["parents"])
        ...
        while len(queue) > 0:
            ...
            if len(parents) == 0:
                ...
                table_key = tuple(
                    [
                        previous_states[_node] if _previous else next_states[_node]
                        for (_node, _previous) in parents
                    ]
                )

...
def random_hmm(*, hidden_states, observed_states, seed=None):
    ...
    S = Util.normalize_dictionary(
        {s: np.random.uniform() for s in hidden_states})

    T = Util.normalize_2d_dictionary(
        {(a, b): np.random.uniform()
         for a in hidden_states for b in hidden_states}
    )
    E = Util.normalize_2d_dictionary(
        {(s, o): np.random.uniform()
         for s in hidden_states for o in observed_states}
    )

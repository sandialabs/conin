...
def Viterbi(obs, hmm):
    ...
    ix_tracker = (
        {}
    )  # this is used in the backwards step to identify the optimal sequence
...
def mv_Viterbi(obs, hmm, cst=None, sat=True):
    ...
    )  # create list of auxillary state to iterate over
    val = {}  # initialize value dictionary
        ...
        for k in hmm.states:
            for r in aux_space:
                ...
                for j in hmm.states:
                    for s in aux_space:
                        curr_val = (
                            val[t - 1, j, s] * hmm.tprob[j, k] * cst.update_fun(r, k, s)
                        )  # j to k
                        if curr_val > max_val:
                            max_val = curr_val
                            argmax = (j, s)
    ...
    for t in range(1, len(obs)):
        for k in hmm.states:
            for r in aux_space:
                ...
                if t == (len(obs) - 1):  # ie. at the last time we add in the constraint
                    val[t, k, r] = (
                        max_val * hmm.eprob[k, obs[t]] * cst.cst_fun(k, r, sat)
                    )
                else:
                    val[t, k, r] = max_val * hmm.eprob[k, obs[t]]
                ix_tracker[t - 1, k, r] = argmax
...
def mv_Viterbi_time(obs, hmm, cst=None, sat=True):
    ...
    for k in hmm.states:
        for r in aux_space:
            val[0, k, r] = (
                cst.init_fun(k, r) * hmm.initprob[k] * hmm.eprob[0, k, obs[0]]
            )
    ...
    for t in range(1, len(obs)):
        for k in hmm.states:
            for r in aux_space:
                ...
                if t == (len(obs) - 1):  # ie. at the last time we add in the constraint
                    val[t, k, r] = (
                        max_val * hmm.eprob[t, k, obs[t]] * cst.cst_fun(r, sat)
                    )
                else:

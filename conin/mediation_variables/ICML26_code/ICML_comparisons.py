...
def generate_random_hmm(
    num_states: int, num_emissions: int, seed: Optional[int] = None
) -> List[torch.Tensor]:
    """
...
def hmm_log_probability(
    hidden_sequence, emission_sequence, initial, transition, emission
):
    """
...
def sample_from_hmm(
    ...
    for attempt in range(max_attempts):
        ...
        if (
            constraint_length is not None
            and current_state == 0
            and not constraint_injected
        ):
...
def find_longest_consecutive_sequence(
    states: List[int], start_state: int = 0, constraint_length: int = None
) -> int:
    """
...
def find_constraint_endpoint(
    states: torch.Tensor, constraint_length: int
) -> Optional[int]:
    """

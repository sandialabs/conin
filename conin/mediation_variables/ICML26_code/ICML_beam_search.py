...
def hmm_beam_search_vectorized(
    ...
    top_initial_scores, top_initial_states = torch.topk(
        log_initial_vector, min(beam_width, N)
    )
    ...
    for t in range(T):
        ...
        prev_states = torch.tensor(
            [path[-1] for path in beam_paths], device=device, dtype=torch.long
        )
        ...
        emission_scores = (
            log_emission_matrix[:, obs].unsqueeze(0).expand(current_beam_size, -1)
        )
        ...
        if bonus_fn is not None:
            if hasattr(bonus_fn, "bonus_matrix"):
                ...
                for i, path in enumerate(beam_paths):
                    prev_state = path[-1]
                    for next_state in range(N):
                        bonus_matrix[i, next_state] = bonus_fn(
                            prev_state, next_state, t
                        )
        ...
        top_k_scores, top_k_indices = torch.topk(
            flat_scores, min(beam_width, flat_scores.shape[0])
        )
        ...
        # Update beam
        new_beam_paths = []
        for beam_idx, state_idx in zip(
            beam_indices.cpu().numpy(), state_indices.cpu().numpy()
        ):
...
def create_sequence_bonus_incentive(
    constraint_length: int, bonus_per_transition: float = 2.0

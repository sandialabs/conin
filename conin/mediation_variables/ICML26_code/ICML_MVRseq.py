...
def create_cst_params(
    cst, hidden_states, cst_params=None, dtype=torch.float32, device="cpu"
):
    ...
    if cst_params:
        ...
        upd_mat = torch.tensor(
            [
                [[upd(k, r, s) for s in m_states] for r in m_states]
...
def create_seqconstraint(K, M):
    ...
    seq_cst = Munch(
        update_fun=update_fun, init_fun=init_fun, eval_fun=eval_fun, m_states=m_states
    )
...
def convertTensor_list(
    ...
    for cst in cst_list:
        ...
        init_mat, eval_mat, upd_mat = create_cst_params(
            cst, hmm.states, cst_params=cst_params, dtype=dtype, device=device
        )
...
def Viterbi_torch_list(
    ...
    """
    input_list = Viterbi_preprocess(
        hmm, cst_list, obs=obs, dtype=dtype, device=device, hmm_params=hmm_params
    )

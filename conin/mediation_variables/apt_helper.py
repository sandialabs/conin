    ...
    for names in names_list:
        ...
        if delay:
            ...
            if names.startswith("apt"):  # no waiting for PRE,POST.
                tprob = {
                    eval(k): round((1 - mu) * v, 5)
                    for k, v in process["transition_probs"].items()
                    if eval(k)[0] not in ["PRE", "POST"]
                }
                add_prob = {
                    eval(k): v
                    for k, v in process["transition_probs"].items()
                    if eval(k)[0] in ["PRE", "POST"]
                }
        ...
        if delay:
            for k in list(
                states
            ):  # need to create a separate copy since we're appending to states
                ...
                for j in states:
                    if (k, j) in tprob.keys():
                        tprob[wait_state, j] = tprob[
                            k, j
                        ]  # 1 - mu factor already applied
                        tprob[wait_state, wait_state] = mu
                eprob[wait_state, None] = 1.0
...
def simulation(hmm, time, ix_list=None, emit_inhom=False):
    ...
    hmm_params, ix_list = hmm2numpy(
        hmm, ix_list=ix_list, return_ix=True, emit_inhom=emit_inhom
    )
    init_prob, tmat, emat = hmm_params
...
def simulation_knowledge(hmm, cst_list, ix_list=None, emit_inhom=False):
    ...
    while x_state != "POST":
        ...
        if (
            hid_emit in notyet_knowledge
        ):  # if knowledge state, gets rid of it from the mask
...
def combined_simulation(apt_hmm, user_list, cst_list=None):
    ...
    for user in user_list:
        total_emits.append(
            simulation(user, T)[1]
        )  # only record the emission of each user

    combined_emits = []
    for t in range(T):
        valid_emits = [
            emits[t] for emits in total_emits if emits[t] is not None
        ]  # exclude the None emits at time t
...
def check_valid(x_list, y_list, cst_list, return_knowledge=False):
    ...
    state_times = {
        c.knowledge_state: 1e10 for c in cst_list
    }  # initialize to some large time
    ...
    if return_knowledge:
        return (
            valid,
            attained_states,
            list(state_times.values()),
        )  # should be in same order as in cst_list
    return True
...
def lapt_mixture(apt_hmm, user_list, length, mix_weights=None, return_ix=False):
    ...
    apt_params, apt_ix_list = hmm2numpy_apt(
        apt, ix_list=[usr_state_ix, emit_ix], return_ix=True
    )
    ...
    initprob, tprob, eprob = numpy2hmm(
        [init_prob, tmat, mix_emat], [apt_state_ix, emit_ix], tol=1e-7, emit_inhom=True
    )
...
def tier_mixture(apt_hmm, user_list, length, mix_weights=None, return_ix=False):
    ...
    apt_hmm = (
        apt_hmm.copy()
    )  # copy so later in-place methods don't overwrite another variable named apt_hmm.
    ...
    # Convert dicts to numpy arrays
    apt_params, apt_ix_list = hmm2numpy_apt(
        apt_hmm, ix_list=[usr_state_ix, emit_ix], return_ix=True
    )
    apt_state_ix = apt_ix_list[0]
    user_params = []
    for user in user_list:
...
def create_noisy_apt(apt_hmm, mix_param, tol=1e-7):
    ...
    apt = copy.deepcopy(
        apt_hmm
    )  # deepcopy since there's still some funkiness going on.
...
def create_tiered_apt(apt_hmm, tol=1e-7):
    ...
    for state1 in new_states:
        for state2 in new_states:
            ...
            h2, e2 = state2
            val = (
                apt.tprob[h1, h2] * apt.eprob[h2, e2]
            )  # P(X_t, Y_t | X_{t-1}, Y_{t-1}) = P(Y_t|X_t)P(X_t | X_{t-1})
...
def arrayConvert(hmm, cst, sat, device=None):
    ...
    for r in aux_space:
        for k in hmm.states:
            ...
            for s in aux_space:
                for j in hmm.states:
                    ind[state_ix[k], aux_ix[r], state_ix[j], aux_ix[s]] = (
                        cst.update_fun(k, r, j, s)
                    )
...
def convertTensor_list(
    hmm, cst_list, sat, dtype=torch.float16, device="cpu", return_ix=False
):
    """
...
def Viterbi_torch_list(
    ...
    hmm_params, cst_params_list, state_ix = convertTensor_list(
        hmm, cst_list, sat, dtype=dtype, device=device, return_ix=True
    )
    ...
    for t in range(1, T):
        ...
        if t == T - 1:
            ...
            val[t] = torch.einsum(
                emit_weights[t], [0], V, kr_indices, *final_ind_list, kr_indices
            ).cpu()
        else:

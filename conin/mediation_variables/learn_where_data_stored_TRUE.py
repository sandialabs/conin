...
def update_fun(k, r, k_past, r_past):
    ...
    """
    m1 = (k == ("DI", ("DS", "syslog/ls"))) or r_past[
        0
    ]  # tracks if the data storage state has occurred yet
    forbidden_transitions = (k_past[0] == "DI" or k_past[0] == "WAIT_DI") and (
        k[0] == "COL" or k[0] == "WAIT_COL"
    )
    forbidden_emissions = k == ("COL", ("DS", "syslog/nano"))
...
def init_fun(k, r):
    ...
    m2 = not (
        k == ("COL", ("DS", "syslog/nano"))
    )  # at first time, can only violate the emission constraint.
...
states = [
    (True, True),
    (False, True),
]  # second variables tracks \tau_A > \tau_B at every t.

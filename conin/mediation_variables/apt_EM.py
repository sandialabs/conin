...
def em_emitweights(apt_hmm, seq_list, mix_param=0.81, dtype=torch.float32):
...
def em_convertTensor(
    ...
    if rand_init:
        ...
        tmat[1, 1] = 1.0  # POST fixed to absorbing
        ...
        emat[(K - L) :] = 0
        emat[(K - L) :, 0] = 1.0  # WAIT states always emit None
        emat[:2] = 0
        emat[:2, 0] = 1.0  # PRE,POST always emit None
    ...
    for cst in cst_list:
        ...
        for r in aux_space:
            for k in hmm.states:
                for e in hmm.emits:
                    curr_combined = (k, e)
                    init_ind[state_ix[k], emit_ix[e], aux_ix[r]] = cst.init_fun(
                        curr_combined, r
                    )
                    for s in aux_space:
                        for j in hmm.states:
    ...
    for ind in ind_list[1:]:
        ...
        imat = torch.einsum(
            "jkesr,jkeuv -> jkesurv", imat, ind
        )  # outer product on last two dims
...
def apt_BW(weight_list, hmm_params, cst_params, debug=False):
    ...
    for C_cpu in weight_list:  # generate the messages for each sequence
        ...
        alpha = torch.empty((T, K, E, M)).to(
            device
        )  # for now on GPU. see if run out of memory
        ...
        for t in range(1, T):  # last message no different since compressed formulation
            ...
            message = torch.einsum(
                "e,ke,jds,jk,jkesr -> ker", C[t], B, alpha[t - 1], A, ind
            )
            norm_message = 1 / message.sum()
        ...
        for t in range(1, T):
            beta[T - 1 - t] = norms[T - 1 - t] * torch.einsum(
                "js,kj,kjrs -> kr", beta[T - t], A, D[T - t]
            )
            # beta[T-1-t] = norms[T-1-t]*torch.einsum('')
        ...
        if debug:  # dat prob P(X,C)
            ...
            all_dat_prob = (
                all_dat_prob / norms
            )  # divide each by the appropriate normalizing constant
        ...
        gamma = gamma / norms.view(
            T, 1, 1
        )  # divide each gamme by appropriate norm constant
        if debug:
            og_gamma = gamma.clone()
            og_xi = torch.einsum(
                "tjds,tkr,jk, tjksr -> t", alpha[: (T - 1)], beta[1:], A, D[1:]
            )  # xi time index starts at 2.
        gamma = gamma.sum(dim=0)  # sum over time
        xi = torch.einsum(
            "tjes,tkr,jk, tjksr -> jk", alpha[: (T - 1)], beta[1:], A, D[1:]
        )  # xi time index starts at 2.
        ...
        log_prob += (
            -1 * torch.log(norms).sum().cpu().item()
        )  # divde by T so it doesn't get too large?
...
def apt_preprocess(apt_hmm, device="cpu"):
    ...
    delay_ix[0, 2:, 1 : (2 + L)] = (
        1  # transitions from OG+WAIT to POST+OG. The 1-mu terms
    )
...
def apt_Mstep(hmm_params, wait_list, ttl_gamma, ttl_xi, fix_mu=None):
    ...
    A[0, : (2 + L)] = (
        ttl_xi[0, : (2 + L)] / ttl_xi[0, : (2 + L)].sum()
    )  # usual update eqn  for PRE

    # update STATE > POST, STATE transitions
    state_xi = torch.einsum(
        "ij,jk -> ik", wait_ix, ttl_xi
    )  # sum messages for OG and WAIT analogue. Transitions to POST, OG only
    ogA = state_xi[:, 1 : (2 + L)] / state_xi[:, 1 : (2 + L)].sum(dim=1, keepdim=True)
    A[1, 1] = 1.0  # POST absorbing
    ...
    B[2 : (2 + L), 1:] = ttl_gamma[2 : (2 + L), 1:] / ttl_gamma[2 : (2 + L), 1:].sum(
        dim=1, keepdim=True
    )
...
def apt_Mstep_old(hmm_params, wait_list, ttl_gamma, ttl_xi):
    ...
    state_xi = torch.einsum(
        "ij,jk -> ik", wait_ix, ttl_xi
    )  # sum messages for K  > K' and WAIT_K > K'
...
def apt_EM(
    ...
    hmm_params, cst_params, state_ix, emit_ix = em_convertTensor(
        ordered_apt,
        cst_list,
        rand_init=rand_init,
        dtype=dtype,
    ...
    weight_list = em_emitweights(
        ordered_apt, seq_list, mix_param=mix_param, dtype=dtype
    )
    ...
    while (it <= max_step) and (change > conv):
        ...
        new_hmm_params = apt_Mstep(
            hmm_params, wait_list, ttl_gamma, ttl_xi, fix_mu=fix_mu
        )
        param_change = sum(
            [
                torch.linalg.norm(old - new).cpu().item()
                for old, new in zip(hmm_params[:2], new_hmm_params[:2])
            ]
        )
...
def apt_EM_v2(
    ...
    while (it <= max_step) and (change > conv):
        ...
        start_time = time.time()
        ttl_gamma, ttl_xi, new_log_prob = apt_BW(
            weight_list, new_hmm_params, cst_params
        )
        run_time = time.time() - start_time
        if timer:
            time_list.append(run_time)

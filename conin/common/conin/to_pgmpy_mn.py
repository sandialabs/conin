...
def convert_conin_to_pgmpy_mn(conin_pgm, check_model=True):
    ...
    if not isinstance(conin_pgm, DiscreteMarkovNetwork):
        raise ValueError(f"Expected conin DiscreteMarkovNetwork, got {type(conin_pgm)}")
    ...
    for conin_factor in conin_pgm.factors:
        ...
        if len(conin_factor.nodes) == 1:
            values = [
                conin_factor.values[v] for v in conin_pgm.states[conin_factor.nodes[0]]
            ]
        else:
            slist = [conin_pgm.states[node] for node in conin_factor.nodes]
            values = [
                conin_factor.values[assignment]
                for assignment in itertools.product(*slist)
            ]

        pgmpy_factor = pgmpy_DiscreteFactor(
            variables=conin_factor.nodes,

    ...
    def __init__(
        ...
        *,
        ...
        if m_space != set(evl.keys()):
            raise InvalidInputError(
                "domain(keys) of evl must match mediation_states"
            )

        if not all(isinstance(v, bool) for v in evl.values()):
            raise InvalidInputError(
                "range(values) of evl must be boolean"
            )
    ...
    def __init__(
        ...
        *,
        ...
        if len(upd) < time_horizon - 1:
            raise InvalidInputError(f"upd length {len(upd)} must be one less than mediation_states length {time_horizon}")
        ...
        for t, m_space in enumerate(mediation_states):
            ...
            if t == 0:
                ...
                if not set(ini.values()) <= m_space:
                    raise InvalidInputError("range(values) of ini must be contained in mediation_states at time 0")
            ...
            if t > 0:
                ...
                if mh_space != set(upd_t_minus_1.keys()):
                    raise InvalidInputError(f"domain(keys) of upd at time {t - 1} must match mediation_states x hidden_states")

                if not set(upd_t_minus_1.values()) <= m_space:
                    raise InvalidInputError(f"range(values) of upd at time {t - 1} must be contained in mediation_states")

            # evl
            evl_t = evl[t]

            if m_space != set(evl_t.keys()):
                raise InvalidInputError(f"domain(keys) of evl at time {t} must match mediation_states")

            if not all(isinstance(v, bool) for v in evl_t.values()):
                raise InvalidInputError(f"range(values) of evl at time {t} must be boolean")

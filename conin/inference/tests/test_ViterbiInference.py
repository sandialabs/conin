...
def test_map_query_Viterbi_hmm1_list_evidence():
    ...
    results = map_query(
        pgm, method="viterbi", variables=None, evidence=observed, show_progress=False, timing=False, start=0, stop=None
    )
...
def test_map_query_Viterbi_unsupported_model():
    ...
    with pytest.raises(TypeError, match="Unsupported model type"):
        map_query(
            pgm, method="viterbi", variables=None, evidence=[], show_progress=False, timing=False, start=0, stop=None
        )
